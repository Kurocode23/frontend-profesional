...
class Db {
  ...
  connect (callback) {
    ...
    return Promise.resolve(setup()).asCallback()
  }
  ...
  saveUser (user, callback) {
    ...
    let tasks = co.wrap(function * () {
      ...
      if (result.errors > 0) {
        return Promise.reject(new Error(result[0]))
      }
  ...
  getImagesByTag (tag, callback) {
    ...
    let tasks = co.wrap(function * () {
      ...
      let images = yield r.db(db).table('images').filter(img => {
        // Devolvemos todas las imagenes que su campo tags contenga el tag que queremos
